...
**状态：** 手动调用、分块累积的课堂清稿和本机多轮自由对话已实现。本文定义 Supabase 接入前的本地旁路能力，不替代 [`session-model.md`](session-model.md) 中未来可同步的 `AssistantBranch` / `AssistantMessage`。
...
- 自由问答请求与结果按 thread 保存到 Dexie；课堂清稿作为可重算阅读投影，按稳定字幕范围保存有序块到 Dexie。
...
延后的 `retranslate` 只读取原文；`ask` 每次读取当前 thread 的完整双语文本、当时全部可用课堂清稿，并携带该 thread 此前成功问答以支持追问；`summarize` 只读取当前待整理块，并附带上一块清稿末尾作为少量连续性参考。自由问答不锁定首次提问时的字幕或清稿范围，每一轮都重新捕获当时的完整会话。原文和实时译文仍是事实证据；课堂清稿只是用于恢复术语、句式、论述结构和显式缺口标记的派生参考，不能升级为独立事实。失败问答不进入后续 history，因为不存在可信的 assistant answer。若完整字幕、课堂清稿、此前问答和当前问题合计超过预算，应明确拒绝，不能静默截断、删除旧轮次或丢掉其中一栏；页面提供显式“清空对话”，由用户决定何时舍弃旧问答后重新开始。
...
`cleanedTranscript` 只供自由对话使用，内容是该 thread 的全部成功分块按顺序拼成的当前完整阅读投影。Dexie version 2 的旧版整场总结是有损摘要，只保留在清稿面板供历史阅读，不得混入该字段冒充清稿。字段缺失时按空串处理以兼容部署切换；服务端只在 `ask` 的准备态中加入它，清稿和重译不得因为请求体携带该字段而读到它。切换 thread 时，页面必须先加载对应清稿再开放提问，不能抢先发送一份静默缺少已知清稿的请求。

完整原文、完整实时译文和完整清稿同时发送会重复表达同一段课程内容，并比只发送双语事实更早耗尽 `ask` 的 120,000 token 预算；此前问答还会继续占用余量。第一版先保留三层上下文并通过真实长课程测量实际 token 数，不预先静默裁剪。若约两到三小时课程稳定超限，优先评估“完整原文 + 完整清稿 + 仅未被清稿覆盖的实时译文尾段”，其次核实模型上下文窗口后调整产品预算，最后才考虑由用户显式选择是否携带清稿；任何方案都不能在页面不知情时删掉上下文。
...
`completed` 还要求存在非空 `output_text`。OpenAI 若报告完成但没有文本，服务端必须转成 `invalid-response`，保留 response/request ID、usage 与有界原始响应；浏览器也拒绝把空文本成功体当成合法结果。面板不得用“模型未返回内容”之类的推测性占位文案覆盖这条证据。
...
页面不在 invocation 里复制完整字幕或清稿；完整文本只存在于原 session、清稿投影状态和正在发送的请求对象中。自由对话按 thread 保存有序问答，每轮结果追加而不是覆盖；切换 thread 或重新载入页面时从 Dexie 恢复，但它仍不是可同步的正式 branch。请求发起前先保存 `requesting` 记录，终态原位更新；页面终止后遗留的 `requesting` 记录转为 `request-outcome-unknown`，明确说明服务端可能已处理并计费，不推断回答、不自动重试。清空对话必须由用户确认并同时删除该 thread 的本地问答记录。每次新问题只把该 thread 此前成功问答作为 history，重新捕获当时的完整字幕和完整可用清稿。同一页面只允许一个旁路调用执行，防止重复点击造成并发费用。旁路工作区位于字幕和诊断之间：课堂清稿与自由对话始终上下排列并使用完整内容宽度，各自保留固定高度滚动区，内容增长不能持续撑高整个页面。每轮结果标出原文、译文、清稿和历史问答规模、`capturedAt`、模型与 token usage，并提供独立复制按钮。第一版不把文本 token 折算成美元，直到项目建立统一且可维护的文本模型价格口径。
