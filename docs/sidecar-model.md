...
模型按任务性质由服务端预设：直接回应真人自由输入的 `ask` 使用 `gpt-5.6-sol`；更重视总结质量、对速度要求适中的 `summarize` 使用 `gpt-5.6-terra`；目标明确、优先速度与成本的 `retranslate` 使用 `gpt-5.6-luna`。第一版浏览器不能覆盖该选择；以后若开放配置，也应从服务端允许的模型集合中选择，而不是接受任意模型名。
