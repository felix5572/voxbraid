...
自用阶段 `ownerId` 可以为空，开放给其他用户时再绑定身份系统。标题在第一句源文形成边界或达到长度上限时由客户端本地生成；若收音提前结束，则用已有的短源文补齐。标题首次生成后保持稳定，不随后续 delta 改写，也不为此调用模型。MVP 不需要显式“结束会话”；离开后仍可继续，只有“新建会话”和“归档”会改变用户所在的容器。
