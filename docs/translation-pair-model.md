...
组长不参与正确性校验，也不会引发重试或失败记录。taskVersion 5 的提示词默认每组表达一个简短语义子句，通常以约 80–180 个 raw 字符为阅读目标；优先沿逗号、分号和冒号原子拆分，不再习惯性地把所有子句重新合成完整长句。原子很短、语义不完整或紧密依赖相邻内容时仍可合并；一组约超过 240 个 raw 字符时优先在已有子句原子处分开。所有长度都只是阅读偏好，模型仍返回长段时直接接受，页面继续以 480 字阈值标记极端「长段」并纳入诊断计数。
...
第四版更换请求级原子协议；第五版只把输出偏好从完整句改为短语义子句。两次变化都不改数据库字段，也不升 Dexie 或 archive schema。现有 v5 投影继续显示；新请求用 taskVersion 5 写入后续 batch，WebSocket chainKey 随版本变化而重建。调试阶段不保留旧协议运行时分支。

archive v5 导出事实、当前修订对照、课堂清稿与自由问答。v4 导入恢复其已有内容并提示不含自由问答；v3 导入恢复事实和修订对照并明确提示缺少清稿；v1/v2 只恢复仍满足当前不变量的 thread、runs 和旧 `segments` 事实。不转换旧投影，也不自动产生 Luna 费用。
...
- 离线或持久化未恢复时不启动。页面隐藏本身不再阻断调度：只要浏览器仍在处理 Realtime delta 与 JavaScript 任务，就继续尽力形成批次并调用 Luna；若 Safari 将页面彻底挂起，则暂停期间无法承诺继续运行，回到前台后从本地事实追赶积压；
- 相同失败 raw 不立即自动重试；连续三次基础设施失败后进入 30 秒退避，后续连续失败依次退避 60 秒、120 秒并以 120 秒为上限，倒计时结束后自动恢复处理新候选。用户可显式立即恢复。run 收尾的同一范围只在服务端明确标记结果可安全重试时，于退避后自动补试一次；断连、超时或无法确认是否已计费的结果不得自动重复同一范围。
- 页面请求必须显式携带切分器版本。服务端仍独立重切并核对原子事实；版本或切分结果不一致时返回 `atomizer-version-mismatch`，页面停止继续自动请求并提示刷新。previousDraft 只是稳定性提示，失配条目由服务端丢弃并通过结构化 warning 回传，不阻断当前事实窗口。
