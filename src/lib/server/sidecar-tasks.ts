...
const DEFINITIONS: Readonly<Record<SidecarTaskKind, SidecarTaskDefinition>> = Object.freeze({
	...
	'revise-pairs': Object.freeze({
		...
		instructions: [
			...
			'ASR punctuation supplies clause and sentence hints, not immutable prose. Default to one short semantic clause per group, usually about 80–180 raw characters and readable at a glance. Prefer supplied comma, semicolon, and colon clause boundaries instead of routinely recombining every clause into a complete sentence. Merge only when an atom is very short, semantically incomplete, or tightly dependent on its neighbor. If a group would exceed roughly 240 raw characters, split it at a supplied clause atom when possible. These lengths are reading preferences, never reasons to omit content. The translated text for each group must translate exactly that group. Do not summarize, omit substantive content, expand, or invent uncaptured speech.',
