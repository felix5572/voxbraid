...
会话和字幕当前保存在访问设备、浏览器与站点域名对应的 IndexedDB 中，而不是 Railway 磁盘。部署后应尽早固定 Railway 或自定义域名；更换域名会形成新的浏览器存储空间。页面提供“导出当前会话 JSON”用于备份，音频本身不保存。跨设备同步与服务端长期保存留到 Supabase 阶段。
