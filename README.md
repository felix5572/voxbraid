...
28. **官方消费只做账号级滚动快照。** 页面通过 Node 一次查询 OpenAI 组织最近 30 天的日级 Realtime 费用桶，并派生近 1 天、7 天和 30 天三个窗口，与当前会话估算并列但不做严格对账。Node 缓存五分钟且只返回脱敏汇总；Admin key 留在服务端，查询失败不能阻断翻译。该接口不单独设计权限，直接继承整站 HTTPS Basic Auth。
